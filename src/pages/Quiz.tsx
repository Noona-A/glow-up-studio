...
const Quiz = () => {
  ...
  return <div className="pt-24 pb-16 min-h-[80vh] flex items-center py-[60px]">
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        {/* Question Card */}
        <div className={`card-luxury p-8 md:p-10 transition-all duration-200 ${isTransitioning ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
          ...
          <div className="space-y-3">
            ...
            return <Button key={i} variant="outline" className={`w-full justify-start text-left h-auto py-4 px-6 transition-all duration-200 ${isSelected ? 'border-primary bg-accent text-foreground' : 'hover:border-primary/50 hover:bg-accent/50'}`} onClick={() => handleAnswer(i)}>
                  <span className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? 'border-primary bg-primary' : 'border-muted-foreground/30'}`}>
        ...
        {/* Back Button */}
        {step > 0 && <Button variant="ghost" className="mt-6 text-muted-foreground hover:text-foreground" onClick={handleBack}>
